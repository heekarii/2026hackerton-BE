...
`POST /uploads/complaint-images`에 `multipart/form-data` 형식으로 이미지
파일을 전송합니다. JPEG, PNG, WebP, GIF를 지원하며 기본 최대 크기는
6MB입니다. 반환된 `file_url`을 민원 등록 요청의 `image_url`로 사용합니다.

Supabase에 `complaint-images` 공개 버킷을 생성하고 `.env.example`의
Storage 환경변수를 설정해야 합니다. `SUPABASE_SERVICE_ROLE_KEY`는
프론트엔드에 노출하거나 Git에 커밋하면 안 됩니다.
